...
const Complete = ({newTask,func,dark}) => {
  ...
  const notify = () => { toast.success("Todo Has Been Removed From The List") };
    ...
    <div className="shadow-lg rounded-lg">
      <h1 className={dark ? 'font-serif bg-slate-500 border-slate-200 text-slate-300 rounded-lg font-bold py-2' : 'font-serif bg-red-300 border-slate-200 border-2 rounded-lg font-bold py-2'}>
        👇 COMPLETED 👇
      </h1>
      {newTask.map((item) => {
        return (
